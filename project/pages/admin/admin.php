...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_product'])) {
    $adminController->deleteProduct($_POST['product_id']);
}
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_notification'])) {
    $adminController->deleteNotification($_POST['notif_id']);
}
...
<div class="main">
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td><?php echo clean($notif['first_name'] . ' ' . $notif['last_name']); ?></td>
                <td><?php echo clean($notif['message']); ?></td>
                <td><?php echo clean($notif['type']); ?></td>
                <td><?php echo clean($notif['date']); ?></td>
                <td>
                    <form method="POST">
                        <input type="hidden" name="notif_id" value="<?php echo clean($notif['id']); ?>">
                        <button type="submit" name="delete_notification" class="btn-delete">Delete</button>
                    </form>
                </td>
            </tr>
            <?php endforeach; ?>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td><?php echo clean($product['id']); ?></td>
                <td><?php echo clean($product['product_name']); ?></td>
                <td><?php echo clean($product['shop_name']); ?></td>
                <td><?php echo clean($product['price']); ?> DA</td>
                <td>
                    <form method="POST">
                        <input type="hidden" name="product_id" value="<?php echo clean($product['id']); ?>">
